...
2. *(Optional)* Adjust the **Temperature** slider — keep at 0 for clinical use.
3. Paste or type a **pathology report** (or any clinical text) into the *User Prompt* box.
4. Click **Analyse Report**.
5. The structured output is displayed as clearly labelled cards — no raw JSON required.
...
- **Temperature 0 is strongly recommended for clinical extraction** — it produces deterministic, consistent JSON output.
        """)
...
with st.sidebar:
    ...
    temperature = st.slider(
        ...
        step=0.1,
        label_visibility="collapsed",
        help="Controls output randomness. 0 = fully deterministic (recommended for clinical extraction). Higher values increase creativity but may reduce JSON consistency.",
    )
    if temperature == 0.0:
        st.caption("✅ 0.0 — deterministic (recommended for clinical use)")
    elif temperature <= 0.5:
        st.caption(f"🟡 {temperature:.1f} — low variability")
    elif temperature <= 1.0:
        st.caption(f"🟠 {temperature:.1f} — moderate variability")
    else:
        st.caption(f"🔴 {temperature:.1f} — ⚠️ high variability, may produce inconsistent JSON")
...
with col_model:
    st.markdown(
        f'<div style="padding:8px 0;color:#64748b;font-size:0.85rem;">🤖 <strong>{selected_model}</strong> &nbsp;·&nbsp; 🌡️ temp <strong>{temperature:.1f}</strong></div>',
        unsafe_allow_html=True,
    )
...
if analyse_btn:
    if not user_prompt.strip():
        ...
        with st.spinner(f"Analysing report with **{selected_model}** (temp={temperature:.1f})…"):
            raw_text = ""
            try:
                client = OpenAI(
                    api_key=API_KEY,
